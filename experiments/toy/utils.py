...
def plot_2d_pareto(trajectories: dict, scale):
    ...
    ax.plot(
        Ys.numpy()[:, 0], Ys.numpy()[:, 1], "-", linewidth=8, color="#72727A",
        label="Pareto Front"
    )  # Pareto front

    for i, tt in enumerate(losses):
        ax.scatter(
            tt[0, 0], tt[0, 1], color='k', s=150, zorder=10, label='Initial Point' if i == 0 else None
        )
        colors = matplotlib.cm.magma_r(np.linspace(.1, .6, tt.shape[0]))
    ...
    legend = ax.legend(
        loc=2, bbox_to_anchor=(-.15, 1.3), frameon=False, fontsize=20, ncol=2
    )
